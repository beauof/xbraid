                        ...
                        braid_Int   periodic_time_grid)
{
   ...
   {
      /* By default, all time grids are non-periodic and the index is out of range */
      proc = -1;
      /* Check for the case of periodic time grids.
       * \todo Make coarse-grid non-periodic and let index fall off the grid.
       */
      if (periodic_time_grid == 1){
         // If the index is out of range to the *right* of the time grid,
         // then we need to communicate with the first time interval.
         // Note: This still works for one processor in time communicator.
         if (index > (npoints-1)){
            proc = 0;
         }
         // If the index is out of range to the *left* of the time grid,
         // then we need to communicate with the last time interval.
         // Note: This still works for one processor in time communicator.
         else if (index < 0){
            proc = nprocs - 1;
         }
               ...
               braid_Int   *proc_ptr)
{
   ...
   braid_Int      periodic_tgrid = _braid_CoreElt(core, periodic_tgrid);// && !(max_levels == level+1); // coarsest grid is always approximated as non-periodic
   ...
   _braid_GetBlockDistProc(npoints, nprocs, index, proc_ptr, periodic_tgrid);
                   ...
                   braid_Int   *ci_ptr)
{
   ...
   if ( _braid_IsCPoint(clower, cfactor) )
   {
      ...
      // \todo Might have to do something for periodic time grids..
               ...
               braid_Int   *refined_ptr)
{
   ...
   braid_Int          periodic_tgrid  = _braid_CoreElt(core, periodic_tgrid);
   ...
   if (npoints > 0)
   {
      ...
      if (ilower > 0)
      {
         _braid_GetBlockDistProc((gupper+1), nprocs, (ilower-1), &prevproc, periodic_tgrid);
   ...
   bptr = send_buffer;
   nsends = -1;
   _braid_GetBlockDistProc((f_gupper+1), nprocs, (r_ilower-1), &prevproc, periodic_tgrid);
   ii = 0;
   for (r_ii = 0; r_ii < r_npoints; r_ii++)
   {
      r_i = r_ilower + r_ii;
      _braid_GetBlockDistProc((f_gupper+1), nprocs, r_i, &proc, periodic_tgrid);
   ...
   for (f_ii = 0; f_ii < f_npoints; f_ii++)
   {
      ...
      {
         i = f_ca[f_ii];
         _braid_GetBlockDistProc((gupper+1), nprocs, i, &proc, periodic_tgrid);
         if (proc != prevproc)
         {
            nrecvs++;
